...
def run_with_path(cmd: list[str], *, check: bool = False) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
...
def export_gbrain() -> str:
    executable = gbrain_path()
    export_dir = ROOT / "gbrain_export"
    if not executable:
        return "gbrain executable was not found; export skipped"

    if export_dir.exists():
        shutil.rmtree(export_dir)
    export_dir.mkdir(parents=True, exist_ok=True)

    result = run_with_path([executable, "export", "--dir", str(export_dir)])
    if result.returncode != 0:
        print(result.stdout, end="")
        raise SystemExit(result.returncode)

    files = sorted(path for path in export_dir.rglob("*.md") if path.is_file())
    manifest = [
        "# GBrain Export",
        "",
        f"Generated: {now_local()}",
        f"Markdown pages: {len(files)}",
        "",
    ...
    return f"exported {len(files)} markdown pages to gbrain_export/"
...
def build_report(date: str, refresh_output: str, self_check_output: str, gbrain_output: str) -> Path:
    ...
    lines.extend(
        [
            ...
            f"- gbrain export: `{latest_matching('gbrain_export/README.md')}`",
            "",
            "## Verification",
            "",
            "```text",
...
def publish(args: argparse.Namespace) -> int:
    ...
    if args.skip_gbrain_export:
        gbrain_output = "skipped"
    else:
        gbrain_output = export_gbrain()
    ...
    report_path = build_report(date, refresh_output, self_check.stdout, gbrain_output)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--skip-gbrain-export", action="store_true")
    return parser
